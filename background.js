...
async function handleSunoPoll() {
  ...
  try {
    ...
    const rawRecords = data?.data?.response?.sunoData || data?.data?.sunoData || [];
    const records = rawRecords.map(song => ({
      ...song,
      audioUrl: song.audioUrl || song.audio_url || null,
      audio_url: song.audioUrl || song.audio_url || null,
      streamAudioUrl: song.streamAudioUrl || song.stream_audio_url || null,
      stream_audio_url: song.streamAudioUrl || song.stream_audio_url || null,
      imageUrl: song.imageUrl || song.image_url || null,
      image_url: song.imageUrl || song.image_url || null,
      modelName: song.modelName || song.model_name || null,
      model_name: song.modelName || song.model_name || null
    }));
